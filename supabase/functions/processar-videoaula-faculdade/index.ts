...
async function generateSobreAula(titulo: string, transcricao: string, area: string): Promise<string> {
  const prompt = `Você é um professor de Direito especializado em criar resumos didáticos.

Com base no título e transcrição da videoaula de ${area} abaixo, crie um texto "Sobre esta aula" que:
1. Explique o tema principal abordado
2. Liste os principais conceitos ensinados
3. Destaque a importância prática do conteúdo para estudantes de Direito
...
async function generateQuestoes(titulo: string, transcricao: string, area: string): Promise<any[]> {
  const prompt = `Você é um professor de Direito criando questões para testar o aprendizado de estudantes.

Com base no título e transcrição da videoaula de ${area} abaixo, crie EXATAMENTE 5 questões de múltipla escolha.
...
async function generateFlashcards(titulo: string, transcricao: string, area: string): Promise<any[]> {
  const prompt = `Você é um professor de Direito especializado em criar flashcards para memorização.

Com base no título e transcrição da videoaula de ${area} abaixo, crie o MÁXIMO de flashcards possível (entre 8 e 15) sobre os conceitos mais importantes.
...
serve(async (req) => {
  ...
  try {
    ...
    const youtubeVideoId = extractVideoId(videoaula.link);
    if (!youtubeVideoId) {
      throw new Error("Could not extract YouTube video ID from link");
    }
    ...
    try {
      transcricao = await fetchYouTubeTranscript(youtubeVideoId);
      ...
      transcricao = `Título: ${videoaula.titulo}. Aula sobre ${videoaula.area || "conceitos jurídicos"}.`;
    ...
    const sobreAula = await generateSobreAula(videoaula.titulo, transcricao, area);
    console.log("'Sobre esta aula' generated");

    // Step 3: Generate flashcards
    console.log("Generating flashcards...");
    const flashcards = await generateFlashcards(videoaula.titulo, transcricao, area);
    console.log("Flashcards generated:", flashcards.length);

    // Step 4: Generate questions
    console.log("Generating questions...");
    const questoes = await generateQuestoes(videoaula.titulo, transcricao, area);
