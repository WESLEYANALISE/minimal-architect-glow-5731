...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Processando PDF para matéria ${materiaId}: ${pdfUrl}`);
    ...
    if (pdfUrl.includes('/folders/') || pdfUrl.includes('/drive/folders')) {
      throw new Error("A URL fornecida é de uma pasta do Google Drive. Por favor, forneça a URL direta do arquivo PDF (ex: https://drive.google.com/file/d/ID_DO_ARQUIVO/view)");
    }

    // Converter URL do Google Drive para download direto
    let downloadUrl = pdfUrl;
    if (pdfUrl.includes('drive.google.com')) {
      // Tentar diferentes padrões de URL do Google Drive
      let fileId = null;
      ...
      // Padrão 3: id=ID em query string
      if (!fileId) {
      ...
      if (fileId) {
        ...
        throw new Error("Não foi possível extrair o ID do arquivo da URL do Google Drive. Verifique se a URL está correta.");
      }
    }
    ...
    if (!pdfResponse.ok) {
      throw new Error(`Erro ao baixar PDF: ${pdfResponse.status} - Verifique se o arquivo está compartilhado publicamente`);
    }

    // Verificar se é realmente um PDF
    const contentType = pdfResponse.headers.get('content-type') || '';
    if (contentType.includes('text/html')) {
      throw new Error("O Google Drive retornou uma página HTML ao invés do PDF. Isso geralmente significa que o arquivo não está compartilhado publicamente ou a URL é de uma pasta.");
    }
    ...
    // Atualizar matéria com total de páginas e status
    await supabase
      .from('conceitos_materias')
      .update({ 
        status_processamento: 'identificando',
