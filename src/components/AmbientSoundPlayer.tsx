...
const AmbientSoundPlayer = () => {
  ...
  // Só mostra se houver um som ambiente ativo
  ...
  return (
    <>
      <div className="fixed bottom-24 md:bottom-8 right-4 z-40 flex flex-col gap-2 items-end animate-fade-in">
        {currentSound && (
          <div className="bg-card border border-accent/20 rounded-full shadow-lg backdrop-blur-sm px-4 py-2 flex items-center gap-3 animate-scale-in">
            <Button
              variant="ghost"
              size="sm"
              onClick={togglePlay}
              className="h-8 w-8 rounded-full p-0 tap-bounce"
            >
              {isPlaying ? (
            ...
            {showVolumeControl && (
              <div className="w-20 volume-slide-enter">
                <Slider
                  value={[volume]}
        ...
        <Button
          ...
          size="lg"
          className={cn(
            "h-14 w-14 rounded-full shadow-lg hover:shadow-accent/50 transition-all tap-bounce",
            isPlaying && currentSound && "sound-pulse-active"
          )}
        >
