...
// Card individual similar ao CursoAulaCard
const VideoaulaAreaCard = ({ area, priority = false, isDesktop = false }: { area: AreaOAB; priority?: boolean; isDesktop?: boolean }) => {
  ...
  return (
    ...
    >
      <div className="flex flex-col rounded-xl bg-card border border-border shadow-lg overflow-hidden h-full">
        ...
        {/* Conteúdo abaixo */}
        <div className="p-2.5 flex flex-col gap-1 flex-1 min-h-[50px]">
...
export const VideoaulasOABAreasCarousel = () => {
  const navigate = useNavigate();
  const { isDesktop } = useDeviceType();

  const { data: areas, isLoading } = useQuery({
    queryKey: ['videoaulas-oab-areas-carousel'],
    queryFn: async () => {
      ...
      // Agrupar por área, pegando a primeira thumbnail/link válida
      const areaMap: Record<string, AreaOAB> = {};
      (data || []).forEach((v: any) => {
        const area = v.area || 'Outros';
        if (!areaMap[area]) {
          // Primeiro vídeo da área - pegar seu link como thumbnail
          areaMap[area] = { 
            area, 
            count: 0, 
            thumbnail: v.link || v.thumb // Preferir link para extrair thumbnail do YouTube
          };
        }
        areaMap[area].count++;
      });
  ...
  // Desktop: grid de 4 colunas | Mobile: carrossel horizontal
  if (isDesktop) {
    return (
...
export default VideoaulasOABAreasCarousel;
