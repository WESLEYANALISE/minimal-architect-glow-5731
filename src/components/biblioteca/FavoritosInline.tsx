...
export const FavoritosInline = () => {
  ...
  const { data: favoritos, isLoading } = useQuery({
    ...
    queryFn: async () => {
      if (!user) return [];
      const { data, error } = await supabase.from("biblioteca_favoritos" as any).select("*").eq("user_id", user.id).order("created_at", { ascending: false });
  ...
  const { data: capasExtras } = useQuery({
    ...
    queryFn: async () => {
      ...
      for (const [tabela, favs] of Object.entries(porTabela)) {
        ...
        try {
          const { data } = await (supabase as any).from(tabela).select(`id, "${capaField}"`).in("id", ids);
          if (data) {
            data.forEach((d: any) => {
  ...
  const removeMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("biblioteca_favoritos" as any).delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["biblioteca-favoritos"] }); toast.success("Removido dos favoritos"); },
  });

  const getCapaUrl = (item: any) => item.capa_url || capasExtras?.[item.id] || null;
        ...
        {Object.entries(grouped).map(([tabela, items]) => (
          <div key={tabela} className="mb-6">
            <div className="flex items-center justify-between mb-2 px-1">
              <h2 className="text-sm font-semibold text-muted-foreground">{BIBLIOTECA_LABEL_MAP[tabela] || tabela.replace("BIBLIOTECA-", "")}</h2>
            ...
            <div className="space-y-2">
              {(items as any[]).map((item: any) => {
                ...
                return (
                  <div key={item.id} className="flex items-center gap-3 p-3 rounded-xl bg-card border border-border/30 cursor-pointer hover:border-amber-500/30 transition-colors group"
                    onClick={() => { const baseRoute = BIBLIOTECA_ROUTE_MAP[item.biblioteca_tabela]; if (baseRoute) navigate(`${baseRoute}/${item.item_id}`); }}>
                    {capa ? <img src={capa} alt={item.titulo} className="w-14 h-20 rounded-lg object-cover flex-shrink-0 shadow-md" /> : (
                      <div className="w-14 h-20 rounded-lg bg-amber-500/10 flex items-center justify-center flex-shrink-0"><BookOpen className="w-6 h-6 text-amber-500/50" /></div>
                    )}
                    <div className="flex-1 min-w-0">
                      ...
                      {item.created_at && (
                        <p className="text-[10px] text-muted-foreground/60 flex items-center gap-1 mt-1">
                          <Calendar className="w-3 h-3" /> Favoritado em {format(parseISO(item.created_at), "dd/MM/yy", { locale: ptBR })}
                        </p>
                      )}
                    </div>
