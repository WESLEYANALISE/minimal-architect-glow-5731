...
import ShareFilmeCardCanvas from "./ShareFilmeCardCanvas";

interface ShareFilmeModalProps {
  open: boolean;
  onClose: () => void;
  filme: FilmeDoDia;
}
...
export default function ShareFilmeModal({ open, onClose, filme }: ShareFilmeModalProps) {
  const { user } = useAuth();
  ...
  const handleConfirm = async () => {
    ...
    try {
      // Generate image from canvas card
      let imagemBase64 = "";
      if (cardRef.current) {
        try {
      ...
      const lines: string[] = [];
      lines.push(`╔══════════════════╗`);
      lines.push(`   🎬 *Filme do Dia*`);
      lines.push(`╚══════════════════╝`);
      lines.push('');
      lines.push(`🎬 *${filme.titulo}*`);
      if (filme.diretor) lines.push(`🎥 _Dirigido por ${filme.diretor}_`);
      if (filme.ano) lines.push(`📅 ${filme.ano}`);
      if (filme.generos?.length) lines.push(`🎭 ${filme.generos.slice(0, 3).join(', ')}`);
      lines.push('');
      if (filme.sinopse) {
        const sinopseResumo = filme.sinopse.length > 400
          ? filme.sinopse.substring(0, 400).trim() + '...'
          : filme.sinopse;
        lines.push(`📝 *Sinopse:*`);
        lines.push(sinopseResumo);
        lines.push('');
      }
      lines.push('━━━━━━━━━━━━━━━━━━');
      lines.push('');
      lines.push(`✨ *${nomeUsuario}* recomendou este filme pra você!`);
      lines.push('');
      if (filme.frase_dia) {
        lines.push(`_"${filme.frase_dia}"_`);
        lines.push('');
      }
      if (filme.tmdb_id) {
        lines.push(`🎬 *Veja mais:*`);
        lines.push(`https://www.themoviedb.org/movie/${filme.tmdb_id}?language=pt-BR`);
      }

      const mensagem = lines.join('\n');
      const imagemUrl = imagemBase64 || filme.poster_path || '';
        ...
        >
          ...
          >
            ...
            <div className="p-5 pb-3">
              <div className="flex items-start gap-4 mb-4">
                {filme.poster_path && (
                  <img
                    ...
                    alt={filme.titulo}
                    className="w-16 h-24 rounded-lg object-cover shadow-lg border border-border/20 flex-shrink-0"
                  />
                )}
                <div className="min-w-0 flex-1 pt-1">
                  ...
                  {filme.diretor && (
                    <p className="text-xs text-muted-foreground mt-0.5">Dir. {filme.diretor}</p>
                  )}
                  {filme.ano && (
              ...
              <div className="flex items-start gap-3 p-3 rounded-xl bg-[#25D366]/10 border border-[#25D366]/20">
                ...
                <p className="text-sm text-foreground/90 leading-relaxed">
                  A <strong className="text-[#25D366]">Evelyn</strong> vai enviar a imagem com a recomendação do filme no WhatsApp! 🎬✨
                </p>
              </div>
            </div>
                  ...
                  {hasPhone ? (
                    <div className="space-y-3">
                      <div className="p-3 rounded-lg bg-muted/50 border border-border/30">
                        ...
                        <p className="text-[10px] text-muted-foreground/70 mt-1">
                          A recomendação será enviada para esse número.
                        </p>
                      </div>
                    </div>
                  ) : (
                    <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
                      <p className="text-xs text-amber-400">
                        Você ainda não cadastrou um número de telefone. Atualize seu perfil para receber a recomendação no WhatsApp.
                      </p>
                    </div>
                  )}
            ...
            <div className="fixed -left-[9999px] -top-[9999px]">
              <ShareFilmeCardCanvas ref={cardRef} filme={filme} nomeUsuario={nomeUsuario} />
            </div>
          </motion.div>
        </motion.div>
      )}
