...
const ProposicaoCarouselCard = ({
  ...
  const isInstitucional = autorNome ? INSTITUTIONAL_REGEX.test(autorNome) : false;
  const [foto, setFoto] = useState<string | undefined>(autorFoto || undefined);

  useEffect(() => {
    let cancelled = false;
    const run = async () => {
      // Não buscar foto se já tem, se não tem nome, ou se é institucional
      if (foto || !autorNome || isInstitucional) return;
  ...
  }, [autorNome, foto, isInstitucional]);

  return (
    <Card
      onClick={onClick}
      className="flex-[0_0_38%] md:flex-[0_0_28%] lg:flex-[0_0_18%] min-w-0 overflow-hidden cursor-pointer transition-all hover:scale-[1.02] hover:shadow-2xl border border-border hover:border-accent/50 shadow-lg bg-card"
    >
      {/* Foto do autor */}
      <div className="aspect-square relative bg-secondary overflow-hidden cover-reflection">
        {isInstitucional ? (
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-amber-900/80 to-amber-950">
            <Landmark className="w-24 h-24 text-amber-400/60" />
          </div>
        ) : foto ? (
          <img
            src={foto}
