...
import { Calendar, Clock, BookMarked, ChevronDown, Sparkles } from "lucide-react";
                    ...
                    {dia.topicos && dia.topicos.length > 0 ? (
                      <div className="space-y-2 pl-10">
                        {dia.topicos.map((topico, tidx) => (
                          <TopicoItem key={tidx} topico={topico} />
                        ))}
                      </div>
                    ) : (
...
function TopicoItem({ topico }: { topico: { horario: string; titulo: string; descricao?: string; aprofundamento?: string } }) {
  const [open, setOpen] = useState(false);
  const hasAprofundamento = topico.aprofundamento && topico.aprofundamento.trim().length > 0;
    ...
    <div className="py-2 border-b border-border/20 last:border-0">
      ...
      {hasAprofundamento && (
        <Collapsible open={open} onOpenChange={setOpen}>
          <CollapsibleTrigger className="flex items-center gap-1.5 mt-2 ml-[92px] text-xs font-medium text-primary hover:text-primary/80 transition-colors">
            <Sparkles className="w-3 h-3" />
            <span>{open ? "Recolher" : "Aprofundar"}</span>
            <ChevronDown className={`w-3 h-3 transition-transform duration-200 ${open ? 'rotate-180' : ''}`} />
          </CollapsibleTrigger>
          <CollapsibleContent>
            <div className="mt-2 ml-[92px] p-3 bg-primary/5 border border-primary/10 rounded-lg">
              <p className="text-xs text-foreground/80 leading-relaxed whitespace-pre-line">
                {topico.aprofundamento}
              </p>
            </div>
          </CollapsibleContent>
        </Collapsible>
