...
import { useAddJuriflixComentario } from "@/hooks/useJuriflixComentarios";
import { useQuery } from "@tanstack/react-query";
import { supabase } from "@/integrations/supabase/client";
import { Loader2, Send, X } from "lucide-react";
import { useNavigate } from "react-router-dom";

interface JuriflixComentarioFormProps {
  juriflixId: number;
  parentId?: string;
  replyingTo?: string;
  onCancel?: () => void;
...
export const JuriflixComentarioForm = ({
  juriflixId,
  parentId,
  replyingTo,
  onCancel,
  onSuccess
}: JuriflixComentarioFormProps) => {
  const { user } = useAuth();
  ...
  const addComentario = useAddJuriflixComentario();
  const maxLength = 500;
