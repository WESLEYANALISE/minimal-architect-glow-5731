...
const ChatProfessora = ({ embedded = false, onBack, initialChatMode }: ChatProfessoraProps) => {
  ...
  }, [setActiveConversationId, loadConversationMessages, setMessages]);
  ...
  return (
    <div className={`flex ${embedded ? 'h-full' : 'h-[100dvh] lg:h-[calc(100vh-4.5rem)]'} relative overflow-hidden bg-background`}>
      {/* Desktop: sidebar + chat side by side */}
      <div className={cn(
        "flex w-full h-full",
        !embedded && "lg:max-h-[calc(100vh-4.5rem)]"
      )}>
        {/* Desktop sidebar */}
        {isDesktop && !embedded && (
          <aside className="hidden lg:flex w-[280px] flex-shrink-0 flex-col border-r border-border/30 bg-[hsl(0,30%,6%)]">
            {sidebarContent}
          </aside>
        )}
        ...
        <div className="flex-1 flex items-center justify-center relative overflow-hidden">
          <div className={cn(
            ...
            !embedded && !isDesktop && "",
            !embedded && isDesktop && "max-w-3xl max-h-[calc(100vh-7rem)] rounded-2xl border border-border shadow-2xl mx-auto my-4"
          )}>
      ...
      {!isDesktop && (
        <Sheet open={showHistorySheet} onOpenChange={setShowHistorySheet}>
          <SheetContent side="left" className="w-[280px] p-0 bg-[hsl(0,30%,6%)] border-r-border/30">
