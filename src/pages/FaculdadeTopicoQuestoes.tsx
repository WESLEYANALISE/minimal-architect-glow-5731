...
const FaculdadeTopicoQuestoes = () => {
  ...
  const { data: topico, isLoading } = useQuery({
    queryKey: ["faculdade-topico-questoes", id],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("faculdade_topicos")
        .select(`
          *,
          disciplina:faculdade_disciplinas(nome)
        `)
        .eq("id", parseInt(id!))
        .single();
  ...
  const playNarracao = async (acertou: boolean) => {
    ...
    try {
      ...
      // Buscar áudio de feedback do cache
  ...
  const verificarResposta = async (index: number) => {
    ...
    // Scroll suave para mostrar o comentário
    setTimeout(() => {
      window.scrollTo({ 
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <div className="flex flex-col items-center justify-center p-4 text-center pt-20">
          ...
          <Button onClick={() => navigate(`/faculdade/topico/${id}`)}>Voltar ao Tópico</Button>
        </div>
      </div>
    );
  }
          ...
          {quizFinalizado ? (
            ...
            >
              ...
              <div className="flex flex-col gap-3">
                <Button onClick={reiniciarQuiz} className="bg-gradient-to-r from-amber-500 to-orange-600">
                  Zerar e Praticar Novamente
                </Button>
                <Button variant="outline" onClick={() => navigate(`/faculdade/topico/${id}`)}>
                  Voltar ao Tópico
                </Button>
              </div>
            </motion.div>
...
export default FaculdadeTopicoQuestoes;
