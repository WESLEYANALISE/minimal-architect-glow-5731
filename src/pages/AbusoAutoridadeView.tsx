  ...
  const uniqueArticleCount = useMemo(() => {
    ...
    articles.forEach(article => {
      if (article["Número do Artigo"]) {
        seen.add(article["Número do Artigo"]);
      }
    });
    return seen.size;
  }, [articles]);
      ...
      <div ref={contentRef} className="max-w-4xl mx-auto overflow-y-auto" style={{ height: 'calc(100vh - 126px)' }}>
        {activeTab === 'artigos' && (
          <div className="flex flex-col h-full">
            {/* Header fixo com ícone, título e contagem */}
            <div className="sticky top-0 z-10 bg-background">
              <div className="px-4 py-4">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {uniqueArticleCount} artigos • {articlesWithAudio.length} com narração
                </p>
              </div>
            ...
            {/* Lista de artigos - área que rola */}
