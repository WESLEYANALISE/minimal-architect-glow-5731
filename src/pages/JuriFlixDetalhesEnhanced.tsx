...
const JuriFlixDetalhesEnhanced = () => {
  ...
  // Auto-enriquecer título que não tem tmdb_id
  ...
  // Auto-enriquecer ao abrir (se não tem tmdb_id)
  useEffect(() => {
  ...
  // Sincronizar streaming (se tem tmdb_id mas não tem onde_assistir)
  useEffect(() => {
  ...
  return (
    <div className="pb-6">
      ...
      <div className="relative">
        ...
        <div className="relative max-w-6xl mx-auto px-4 -mt-32 md:-mt-40">
          <div className="flex flex-col items-center md:items-start md:flex-row gap-6">
            {/* Poster com botão favoritar */}
            <div className="relative w-48 md:w-64 h-72 md:h-96 rounded-xl overflow-hidden shrink-0 bg-secondary shadow-2xl ring-1 ring-white/10">
              <img
                src={posterUrl}
                alt={titulo.nome}
      ...
      <div className="max-w-6xl mx-auto px-4 mt-8 space-y-8">
                   ...
                   <div className="grid grid-cols-2 gap-3">
                     {ondeAssistir.flatrate.map((provider: any, i: number) => {
                       ...
                       return (
                        ...
                        >
                          {logoUrl && (
                            <img src={logoUrl} alt={provider.provider_name} className="w-8 h-8 rounded-lg" />
                          )}
        ...
        {/* Tabs: Sinopse / Elenco / Comentários / Avaliações */}
        <Tabs defaultValue="sinopse" className="w-full">
          <TabsList className="w-full h-10 bg-card/50">
            <TabsTrigger value="sinopse" className="flex-1 data-[state=active]:bg-red-600 data-[state=active]:text-white text-xs sm:text-sm">
              Sinopse
            </TabsTrigger>
            <TabsTrigger value="elenco" className="flex-1 data-[state=active]:bg-red-600 data-[state=active]:text-white text-xs sm:text-sm">
              Elenco
            </TabsTrigger>
            <TabsTrigger value="comentarios" className="flex-1 data-[state=active]:bg-red-600 data-[state=active]:text-white text-xs sm:text-sm">
              Comentários
            </TabsTrigger>
            <TabsTrigger value="avaliacoes" className="flex-1 data-[state=active]:bg-red-600 data-[state=active]:text-white text-xs sm:text-sm">
              Avaliações
            </TabsTrigger>
          </TabsList>
             ...
             {titulo.beneficios && (
              <Card className="p-6 bg-gradient-to-br from-accent/5 to-accent/10 border-accent/20">
                <h2 className="text-xl font-bold mb-4 text-accent">
                  🎓 Por que assistir este título?
                </h2>
                <div className="space-y-3">
                  {titulo.beneficios.split(/\n|\.(?=\s)/).filter((s: string) => s.trim().length > 10).map((paragrafo: string, i: number) => (
                    <p key={i} className="leading-relaxed text-sm">
                      {paragrafo.trim().endsWith('.') ? paragrafo.trim() : paragrafo.trim() + '.'}
                    </p>
                  ))}
                </div>
              </Card>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  {titulo.diretor && (
                    <div>
                      ...
                      <p className="font-medium">{titulo.diretor}</p>
                    </div>
                  )}
                  {titulo.idioma_original && (
                    <div>
                      <p className="text-muted-foreground">Idioma Original</p>
                      <p className="font-medium">{titulo.idioma_original}</p>
                    </div>
                  )}
                  {titulo.orcamento && titulo.orcamento > 0 && (
                    <div>
                      <p className="text-muted-foreground">Orçamento</p>
                      <p className="font-medium">${(titulo.orcamento / 1000000).toFixed(1)}M</p>
                    </div>
                  )}
                  {titulo.bilheteria && titulo.bilheteria > 0 && (
                    <div>
                      <p className="text-muted-foreground">Bilheteria</p>
                      <p className="font-medium">${(titulo.bilheteria / 1000000).toFixed(1)}M</p>
                    </div>
                  )}
                </div>
              </Card>
            ...
            <Card className="p-6">
              {elenco.length > 0 ? (
                ...
                <p className="text-center text-muted-foreground py-8">
                  Informações de elenco não disponíveis
                </p>
              )}
            </Card>
          </TabsContent>
          ...
          <TabsContent value="comentarios" className="mt-4">
            <Card className="p-6">
              <JuriflixComentariosSection juriflixId={titulo.id} />
            </Card>
          </TabsContent>

          <TabsContent value="avaliacoes" className="mt-4">
            <Card className="p-6">
              <JuriflixAvaliacaoSection juriflixId={titulo.id} />
            </Card>
          </TabsContent>
        </Tabs>
