...
const BibliotecaOratoriaLivro = () => {
  ...
  const isBookFree = useIsBookFree(livroId ? parseInt(livroId) : undefined, 'oratoria');
  ...
  if (!livro) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4">
        ...
        <Button onClick={() => navigate('/biblioteca-oratoria')}>Voltar</Button>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-b from-background via-background to-accent/5 pb-20 animate-fade-in">
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="flex flex-col items-center">
          ...
          <div className="relative w-56 md:w-72 aspect-[2/3] mb-8 rounded-xl overflow-hidden shadow-2xl hover:shadow-accent/50 transition-shadow duration-300">
            {livro.imagem ? (
              <img
                src={livro.imagem}
                alt={livro.livro || ""}
            ...
            {/* Badge Premium - só para não-premium */}
            {!canAccess && (
          ...
          <div className="w-full max-w-2xl text-center space-y-6">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold mb-2">{livro.livro}</h1>
              {livro.autor && (
