...
import { ArrowLeft, PlayCircle, Crown } from "lucide-react";
      ...
      <div className="max-w-[600px] lg:max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="relative space-y-6">
          ...
          {temas.map((temaData, index) => {
            ...
            return (
              <div key={index} className="relative pl-8 animate-fade-in-up" style={{
                animationDelay: `${index * 0.12}s`,
                animationFillMode: 'backwards'
              }}>
                {/* Marcador colorido com animação */}
                <div className={`absolute left-0 top-4 w-7 h-7 rounded-full ${isLocked ? 'bg-muted' : corArea} border-4 border-background flex items-center justify-center shadow-xl animate-bounce-in`} style={{
                ...
                <button 
                  onClick={() => {
                    ...
                    navigate(`/iniciando-direito/${encodeURIComponent(areaDecoded)}/aula/${encodeURIComponent(temaData.tema)}`);
                  }} 
                  className={`w-full text-left relative overflow-hidden backdrop-blur-sm border-2 border-border/50 rounded-lg shadow-xl transition-all duration-300 group ${isLocked ? 'opacity-60' : 'hover:border-primary hover:shadow-2xl hover:scale-[1.02]'}`}
                  style={{
                    background: `linear-gradient(135deg, hsl(var(--card)) 0%, hsl(var(--card)) 70%, ${corArea.replace('bg-', '')} 100%)`
                  }}>
                  {/* Crown badge for locked */}
                  {isLocked && (
                    <div className="absolute top-3 left-3 z-20 flex items-center gap-1.5 bg-gradient-to-r from-amber-500 to-orange-600 text-white px-2.5 py-1 rounded-full text-xs font-semibold shadow-lg">
                  ...
                  {temaData['capa-aula'] && <div className="relative h-40 overflow-hidden bg-muted">
                      <img src={temaData['capa-aula']} alt={temaData.tema} className={`w-full h-full object-cover transition-transform duration-500 ${isLocked ? 'grayscale-[30%]' : 'group-hover:scale-110'}`} loading={index < 3 ? "eager" : "lazy"} decoding="async" fetchPriority={index === 0 ? "high" : "auto"} onError={e => {
                  ...
                  <div className="p-5 relative z-10">
                    ...
                    <div className="flex items-center gap-2 text-xs text-muted-foreground group-hover:text-foreground transition-colors">
                      ...
                      <span>{isLocked ? 'Conteúdo Premium' : 'Videoaula + Conteúdo Detalhado'}</span>
                    </div>

                    <div className="mt-3 text-right">
                      <span className={`text-xs font-semibold group-hover:underline inline-flex items-center gap-1 ${isLocked ? 'text-amber-500' : 'text-primary'}`}>
                        {isLocked ? 'Desbloquear →' : 'Começar aula →'}
                      </span>
                    </div>
                  </div>
                </button>
