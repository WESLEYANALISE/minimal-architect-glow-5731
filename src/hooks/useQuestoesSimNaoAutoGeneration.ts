...
import { questoesSimNaoGenManager } from '@/lib/questoesSimNaoGenerationManager';
...
export function useQuestoesSimNaoAutoGeneration({ area, temas, enabled, onProgress }: Props) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = questoesSimNaoGenManager.subscribe(area, (state) => {
      if (!state) {
  ...
  useEffect(() => {
    ...
    if (questoesSimNaoGenManager.isRunning(area)) return;
    ...
    setTimeout(() => {
      if (questoesSimNaoGenManager.isRunning(area)) return;
      questoesSimNaoGenManager.startArea(area, pendentes.map((tema) => ({ tema: tema.tema })));
    }, 1200);
  }, [enabled, temas, area]);
