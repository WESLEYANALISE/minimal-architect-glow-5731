...
async function fetchTemasSimNaoData(area: string): Promise<TemaData[]> {
  ...
  const temasMap: Record<
    string,
    { nomeOriginal: string; ordem: number; subtemas: Set<string> }
  > = {};
  ...
  (resumoData || []).forEach((row) => {
    ...
    if (!temasMap[temaNorm]) {
      temasMap[temaNorm] = {
        nomeOriginal: row.tema.trim(),
        ordem: ordemCounter++,
        subtemas: new Set<string>(),
      };
    }
  ...
  const newPrefix = `questoes-sn:${encodedArea}:`;
  ...
  const cacheRows = [...(newCacheResult.data || []), ...(legacyCacheResult.data || [])];

  const generatedByTemaSubtema = new Set<string>();
  const generatedBySubtemaLegacy = new Set<string>();

  cacheRows.forEach((row) => {
    const materia = row.materia || '';

    if (materia.startsWith(newPrefix)) {
      const rest = materia.slice(newPrefix.length);
    ...
    if (materia.startsWith(legacyPrefix)) {
      const subtemaLegacy = materia.slice(legacyPrefix.length);
      if (subtemaLegacy) {
        generatedBySubtemaLegacy.add(normalizar(subtemaLegacy));
      }
    }
  });
  ...
  return Object.entries(temasMap)
    .map(([temaNorm, item]) => {
      ...
      item.subtemas.forEach((subtemaNorm) => {
        if (
          generatedByTemaSubtema.has(`${temaNorm}::${subtemaNorm}`) ||
          generatedBySubtemaLegacy.has(subtemaNorm)
        ) {
          subtemasGerados += 1;
        }
      });
      ...
      return {
        ...
        subtemasGerados,
        totalSubtemas,
        totalQuestoes: subtemasGerados * 10,
        progressoPercent,
        ordem: item.ordem,
      };
    })
...
export function useQuestoesSimNaoTemas(area: string) {
  const cacheKey = `questoes-sim-nao-temas-${normalizar(area || '')}`;

  const { data, isLoading, isFetching, error, refresh } = useInstantCache<TemaData[]>({
    cacheKey,
    queryFn: () => fetchTemasSimNaoData(area),
    enabled: !!area,
