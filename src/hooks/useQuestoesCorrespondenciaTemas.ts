...
async function fetchTemasCorrespondenciaData(area: string): Promise<TemaData[]> {
  ...
  const prefix = `questoes-corresp:${encodeURIComponent(area)}:`;
  ...
  (cacheRows || []).forEach((row) => {
    ...
    generatedByTemaSubtema.add(`${normalizar(safeDecode(temaEnc))}::${normalizar(safeDecode(subtemaEnc))}`);
  });
...
export function useQuestoesCorrespondenciaTemas(area: string) {
  const cacheKey = `questoes-correspondencia-temas-${normalizar(area || '')}`;

  const { data, isLoading, isFetching, error, refresh } = useInstantCache<TemaData[]>({
    cacheKey,
    queryFn: () => fetchTemasCorrespondenciaData(area),
    enabled: !!area,
