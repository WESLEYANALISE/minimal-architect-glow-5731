...
interface UseCategoriasAutoGenerationProps {
  materiaId: number | null;
  topicos: Topico[] | undefined;
  enabled?: boolean;
}

export const useCategoriasAutoGeneration = ({
  materiaId,
  topicos,
  enabled = true,
  ...
  const startBatchGeneration = useCallback(async () => {
    ...
    const promises = pendingBatch.map(async (topico) => {
      try {
        const { error } = await supabase.functions.invoke("gerar-conteudo-categorias", {
        ...
        console.error(`[CategoriasAutoGen] Erro:`, err);
      }
    });
