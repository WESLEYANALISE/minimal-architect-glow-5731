...
import { correspondenciaGenManager } from '@/lib/correspondenciaGenerationManager';
...
export function useCorrespondenciaAutoGeneration({ area, temas, enabled, onProgress }: Props) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = correspondenciaGenManager.subscribe(area, (state) => {
      if (!state) {
  ...
  useEffect(() => {
    ...
    if (correspondenciaGenManager.isRunning(area)) return;
    ...
    setTimeout(() => {
      if (correspondenciaGenManager.isRunning(area)) return;
      correspondenciaGenManager.startArea(area, pendentes.map((tema) => ({ tema: tema.tema })));
    }, 1200);
  }, [enabled, temas, area]);
