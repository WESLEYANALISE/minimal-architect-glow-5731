...
class QuestoesCasoPraticoGenerationManager {
  ...
  async startArea(area: string, temasPendentes: { tema: string }[]) {
    ...
    const jobs: GenerationJob[] = temasPendentes.map((t) => ({ area, tema: t.tema, status: 'pending' }));
    const state: AreaGenerationState = { area, jobs, totalGerados: 0, isRunning: true, startedAt: Date.now() };
  ...
  private async processParallel(area: string) {
    ...
    const runNext = async (): Promise<void> => {
      ...
      try {
        ...
        console.error(`❌ [CasoPraticoGen] Erro em ${nextJob.tema}:`, error);
  ...
  private async generateForTema(area: string, tema: string): Promise<number> {
    ...
    while (hasMore && attempts < MAX_ATTEMPTS_PER_TEMA) {
      attempts++;
      const { data, error } = await supabase.functions.invoke('gerar-questoes-caso-pratico', {
        body: { area, tema },
      });
      if (error) throw error;
      totalGerados += data?.questoes_geradas || 0;
      hasMore = !data?.geracao_completa;
      if (hasMore) await new Promise((resolve) => setTimeout(resolve, 1200));
    }
    return totalGerados;
  }
}

export const questoesCasoPraticoGenManager = new QuestoesCasoPraticoGenerationManager();
