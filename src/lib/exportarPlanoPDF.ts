...
export const exportarPlanoPDF = async ({
  ...
  const addText = (
    ...
    const { fontSize = 11, fontStyle = "normal", color = [51, 51, 51], indent = 0 } = options;
  ...
  const addSection = (title: string) => {
    ...
    addText(title, { fontSize: 13, fontStyle: "bold", color: [59, 59, 59] });
    yPosition += 4;
  };
  ...
  if (plano.cronograma && plano.cronograma.length > 0) {
    ...
    plano.cronograma.forEach((semana) => {
      ...
      addText(`Semana ${semana.semana}: ${semana.titulo}`, { fontSize: 11, fontStyle: "bold", color: [67, 56, 202] });
      yPosition += 3;

      semana.dias.forEach((dia) => {
        addNewPageIfNeeded(15);
        addText(`${dia.dia} (${dia.cargaHoraria})`, { fontSize: 10, fontStyle: "bold", indent: 5 });

        dia.topicos.forEach((topico) => {
          addNewPageIfNeeded(10);
          const textoTopico = topico.horario ? `${topico.horario} - ${topico.titulo}` : topico.titulo;
          addText(textoTopico, { fontSize: 10, indent: 12, color: [71, 71, 71] });
          if (topico.descricao) {
            addText(topico.descricao, { fontSize: 9, indent: 15, color: [100, 100, 100] });
          }
          if (topico.aprofundamento) {
            addNewPageIfNeeded(15);
            addText(`↳ ${topico.aprofundamento}`, { fontSize: 8, indent: 18, fontStyle: "italic", color: [90, 90, 130] });
          }
        });
        yPosition += 2;
      });
  ...
  // ========== LIVROS RECOMENDADOS ==========
  if (plano.livrosRecomendados && plano.livrosRecomendados.length > 0) {
    addSection("LIVROS RECOMENDADOS NO APP");
    plano.livrosRecomendados.forEach((livro) => {
      addNewPageIfNeeded(10);
      addText(`• ${livro.titulo}`, { fontSize: 10, fontStyle: "bold", indent: 5 });
      if (livro.area) {
        addText(`Área: ${livro.area}`, { fontSize: 9, indent: 10, color: [100, 100, 100] });
      }
      yPosition += 2;
    });
  }
  ...
  if (plano.materiais && plano.materiais.length > 0) {
    ...
    plano.materiais.forEach((material) => {
      addNewPageIfNeeded(15);
      addText(`[${material.tipo}] ${material.titulo}`, { fontSize: 10, fontStyle: "bold", indent: 5 });
      if (material.autor) addText(`Por: ${material.autor}`, { fontSize: 9, indent: 10, color: [100, 100, 100] });
      if (material.detalhes) addText(material.detalhes, { fontSize: 9, indent: 10, color: [80, 80, 80] });
      yPosition += 2;
    });
  }
  ...
  if (plano.estrategias && plano.estrategias.length > 0) {
    ...
    plano.estrategias.forEach((estrategia, idx) => {
      addNewPageIfNeeded(15);
      addText(`${idx + 1}. ${estrategia.titulo}`, { fontSize: 10, fontStyle: "bold", indent: 5 });
      addText(estrategia.descricao, { fontSize: 9, indent: 10, color: [71, 71, 71] });
      yPosition += 2;
    });
  }
  ...
  if (plano.checklist && plano.checklist.length > 0) {
    ...
    plano.checklist.forEach((item) => {
      addNewPageIfNeeded(10);
      addText(`[ ] Semana ${item.semana}: ${item.meta}`, { fontSize: 10, indent: 5 });
    });
  }
  ...
  if (plano.revisaoFinal.descricao) {
    ...
    if (plano.revisaoFinal.simulado) {
      ...
      addText("Simulado Final", { fontSize: 10, fontStyle: "bold", color: [180, 100, 50] });
      addText(`Duracao: ${plano.revisaoFinal.simulado.duracao} | Formato: ${plano.revisaoFinal.simulado.formato}`, { fontSize: 9, color: [100, 100, 100] });
    }
  }
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text("Gerado por Direito Premium", pageWidth - margin, pageHeight - 10, { align: "right" });
  }
  ...
  const { data: urlData } = supabase.storage.from('pdfs-educacionais').getPublicUrl(filePath);
