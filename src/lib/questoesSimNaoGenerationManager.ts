...
class QuestoesSimNaoGenerationManager {
  ...
  private async processParallel(area: string) {
    ...
    const runNext = async (): Promise<void> => {
      ...
      try {
        ...
        console.error(`❌ [QuestoesSimNaoGen] Erro em ${nextJob.tema}:`, error);
  ...
  private async generateForTema(area: string, tema: string): Promise<number> {
    ...
    while (hasMore && attempts < MAX_ATTEMPTS_PER_TEMA) {
      ...
      const { data, error } = await supabase.functions.invoke('gerar-questoes-sim-nao', {
        body: { area, tema },
      });
...
export const questoesSimNaoGenManager = new QuestoesSimNaoGenerationManager();
